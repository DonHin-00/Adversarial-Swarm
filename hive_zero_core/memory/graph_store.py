...
class HeteroLogEncoder(nn.Module):
    ...
    def _update_old(self, logs: List[Dict], mitre_context: Optional[List[Dict]] = None) -> HeteroData:  # noqa: PLR0912, PLR0915
        ...
        port_indices = torch.tensor(
            [p for p, _ in sorted(local_port_map.items(), key=lambda x: x[1])], dtype=torch.long, device=device
        )
        ...
        if ip_port_src:
            data["ip", "binds", "port"].edge_index = torch.tensor(
                [ip_port_src, ip_port_dst], dtype=torch.long, device=device
            )
        else:
            data["ip", "binds", "port"].edge_index = torch.empty((2, 0), dtype=torch.long, device=device)

        if ip_tech_src:
            data["ip", "exhibits", "technique"].edge_index = torch.tensor(
                [ip_tech_src, ip_tech_dst], dtype=torch.long, device=device
            )
        else:
