...
class AccessController:
    ...
    def register_actor(self, actor_id: str, role_name: str,
                      session_timeout_minutes: int = 60) -> Actor:
        """
    ...
    def authorize_operation(self, actor_id: str, operation: OperationType) -> bool:
        ...
        if operation in actor.role.rate_limits:
            ...
            if current_rate >= rate_limit:
                logger.warning(f"Rate limit exceeded for {actor_id}: {operation.value} "
    ...
    def get_actor_stats(self, actor_id: str) -> Dict:
        ...
        return {
            ...
            'operation_counts': {
                op.value: len(times) 
                for op, times in actor.operation_counts.items()
            }
        }
