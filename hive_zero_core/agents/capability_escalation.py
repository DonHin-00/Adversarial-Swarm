...
class CapabilityRegistry:
    ...
    def get_capabilities_by_merge_count(self, merge_count: int) -> List[Capability]:
        ...
        return [cap for cap in self.capabilities.values() if cap.unlock_threshold <= merge_count]
...
class CapabilityManager:
    ...
    def unlock_capabilities_for_unit(
        self, unit_id: str, merge_count: int, existing_capabilities: Optional[Set[str]] = None
    ) -> List[Capability]:
        """
    ...
    def get_unit_capabilities(self, unit_id: str) -> List[Capability]:
        ...
        return [
            self.registry.get_capability(name)
    ...
    def calculate_power_multiplier(self, unit_id: str) -> float:
        ...
        final_multiplier = (
            base_power * (1.0 + capability_bonus * 0.1) * min(synergy_multiplier, 3.0)
        )
        ...
        logger.debug(
            f"Power multiplier for {unit_id[:8]}: {result:.2f}x "
    ...
    def get_statistics(self) -> Dict:
        ...
        return {
            ...
            "avg_capabilities_per_unit": (
                ...
                / len(self.unit_capabilities)
                if self.unit_capabilities
                else 0
            ),
        }
