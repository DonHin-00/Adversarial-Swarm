...
class VariantArchive:
    ...
    def add_variant(self, variant: ArchivedVariant) -> bool:
        ...
        logger.info(
            f"Archived variant {variant.variant_id} (gen={variant.generation}, fitness={variant.fitness:.4f})"
        )
        return True
    ...
    def get_by_tags(self, tags: List[str]) -> List[ArchivedVariant]:
        ...
        return [v for v in self.variants.values() if all(tag in v.tags for tag in tags)]
    ...
    def load_from_disk(filepath: str) -> Optional["VariantArchive"]:
        ...
        try:
            ...
            archive = VariantArchive(max_size=data["max_size"], auto_prune=data["auto_prune"])
