...
from hive_zero_core.agents.stealth_backpack import StealthBackpack, StealthLevel, CollectionMode
...
class Variant:
    ...
    def _initialize_specialization(self):
        ...
        self.specialization_traits = {
            k: min(v * tier_multiplier, 1.0) for k, v in self.specialization_traits.items()
        }
    ...
    def _initialize_backpack(self):
        ...
        self.backpack = StealthBackpack(
            stealth_level=stealth_level, collection_mode=collection_mode
        )

        logger.debug(
            f"Variant {self.variant_id} backpack initialized: "
    ...
    def all_jobs_complete(self) -> bool:
        ...
        return all(job.status in [JobStatus.COMPLETED, JobStatus.FAILED] for job in self.jobs)
    ...
    def die(self):
        ...
        if self.backpack:
            ...
            logger.debug(
                f"Variant {self.variant_id} harvested backpack intelligence: "
                f"{backpack_intel['metrics']['total_collected']} items"
            )

        logger.info(
            f"🪦 Variant {self.variant_id} (role={self.role.value}, tier={self.tier.name}) "
    ...
    def __repr__(self):
        ...
        return (
            f"Variant{status}({self.variant_id[:8]}, role={self.role.value}, "
...
class IntelligenceHub:
    ...
    def receive_intelligence(self, variant_report: Dict[str, Any]):
        ...
        logger.info(
            f"📥 Received intelligence from variant {variant_report['variant_id'][:8]} "
    ...
    def get_collective_intelligence(self, role: Optional[VariantRole] = None) -> Dict[str, Any]:
        ...
        return {
            "all_roles": {role.value: self.role_statistics.get(role, {}) for role in VariantRole},
...
class VariantBreeder:
    ...
    def breed_variant(self, parent: SwarmUnit, role: VariantRole) -> Variant:
        ...
        logger.info(
            f"🧬 Bred {variant} from parent (tier={parent_tier.name}, "
            f"merges={parent.merge_count}) with {max_jobs} jobs"
        )

        return variant

    def cross_breed_variants(
        self, parent1: SwarmUnit, parent2: SwarmUnit, role1: VariantRole, role2: VariantRole
    ) -> Variant:
        """
        ...
        logger.info(
            f"🧬🧬 Cross-bred hybrid {variant} from {role1.value}+{role2.value} "
    ...
    def _inject_intelligence(self, variant: Variant, role: VariantRole):
        ...
        if "statistics" in collective_intel:
            ...
            if stats:
                ...
                logger.debug(
                    f"Injected intelligence into {variant.variant_id[:8]}: "
    ...
    def spawn_generation(
        self, parents: List[SwarmUnit], roles: List[VariantRole], cross_breed_rate: float = 0.3
    ) -> List[Variant]:
        """
        ...
        for parent in parents:
            ...
            if (
                SecureRandom.random_float() < cross_breed_rate
                and len(parents) > 1
                and len(roles) > 1
            ):
        ...
        logger.info(
            f"🧬 Spawned generation of {len(variants)} variants from {len(parents)} parents"
        )
